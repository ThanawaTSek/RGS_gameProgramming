...
public class Turret : Building
{
    ...
    [SerializeField] TurrentState state = TurrentState.Idle;
    
    // Start is called before the first frame update
    void Start()
    {
        unitLayerMask = LayerMask.GetMask("Unit");
        InvokeRepeating("CheckForAttack",0,0.5f);
    }
    ...
    private void CheckForAttack()
    {
        ...
        if (state == TurrentState.Defending)
            ShootAtEnemy();
    }
